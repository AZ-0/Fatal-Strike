...
public class KeyboardMenu extends Menu
{
	...
	public void prepare()
	{
		...
		{
			c.setFont(FatalStrike.GameManager.FONT_GUI);
			c.getAppearance().setForeColor(theme.fore);
			c.getAppearance().setBackgroundColor1(theme.bg);
			c.getAppearance().setTransparentBackground(false);
			c.getAppearance().setTextAntialiasing(RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
			
			c.getAppearanceHovered().setForeColor(theme.hoveredFore);
			c.getAppearanceHovered().setBackgroundColor1(theme.hoveredBg);
	...
	public boolean isLocked() {
		return this.isSuspended() || !this.isVisible() || !this.isEnabled() || Game.time().since(lastInput) < DELAY; }
	...
	public static enum Theme
	{
		...
		public Color getHoveredForeColor() { return hoveredFore; }
		public Color getHoveredBackgroundColor() { return hoveredBg; }
	}
}
